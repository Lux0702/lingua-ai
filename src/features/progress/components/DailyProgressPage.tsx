        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <div className="flex gap-1">
              <Button variant="outline" size="icon-sm" aria-label="Tháng trước" onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1, 1))}>
                <ChevronLeft />
              </Button>
              <Button variant="outline" size="icon-sm" aria-label="Tháng sau" onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 1))}>
                <ChevronRight />
              </Button>
            </div>
          </CardHeader>
          <CardContent>
            ...
            <div className="mt-2 grid grid-cols-7 gap-2">
              {calendarDays.map((day, index) => {
                ...
                return (
                  <button key={key} type="button" onClick={() => selectDate(day)} className={cn("relative flex aspect-square items-center justify-center rounded-lg text-sm font-medium transition-colors hover:bg-muted", isSelected && "ring-2 ring-primary ring-offset-2", isToday && !isSelected && "bg-primary/10 text-primary", completed && "bg-emerald-500 text-white hover:bg-emerald-600")}>
            ...
            <div className="mt-5 flex items-center gap-4 text-xs text-muted-foreground">
              <span className="flex items-center gap-1.5"><i className="size-3 rounded-sm bg-emerald-500" />Đã hoàn thành</span>
              <span className="flex items-center gap-1.5"><i className="size-3 rounded-sm bg-primary/10" />Hôm nay</span>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-6">
          <Card>
            <CardContent className="pt-4">
              <p className="text-sm text-muted-foreground">Đã hoàn thành trong tháng</p>
              <p className="mt-1 text-4xl font-bold">{completedThisMonth}<span className="text-lg font-medium text-muted-foreground"> ngày</span></p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
              <CardTitle className="capitalize text-lg">{formatSelectedDate(selectedDate)}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <button type="button" onClick={() => updateSelectedEntry({ completed: !selectedEntry.completed })} className={cn("flex w-full items-center gap-3 rounded-lg border p-3 text-left transition-colors", selectedEntry.completed ? "border-emerald-500 bg-emerald-50 dark:bg-emerald-950/30" : "hover:bg-muted")}>
                <span className={cn("flex size-5 items-center justify-center rounded border", selectedEntry.completed && "border-emerald-500 bg-emerald-500 text-white")}>{selectedEntry.completed && <Check className="size-4" />}</span>
                <span className="font-medium">Đã học hôm nay</span>
              </button>
              <div className="space-y-2">
                <label htmlFor="daily-note" className="flex items-center gap-2 text-sm font-medium"><PencilLine className="size-4" />Hôm nay bạn đã học gì?</label>
                <textarea id="daily-note" value={selectedEntry.note} onChange={(event) => updateSelectedEntry({ note: event.target.value })} placeholder="Ví dụ: 20 từ vựng tiếng Nhật, mẫu câu chào hỏi..." className="min-h-28 w-full resize-y rounded-lg border bg-transparent px-3 py-2 text-sm outline-none placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50" />
              </div>
            </CardContent>
          </Card>
        </div>
